...
# П'яте завдання: Обробка колекцій

## Опис проекту
Цей модуль демонструє реалізацію шаблонів проектування:
- **Command** - для обробки команд користувача через консольний інтерфейс
- **Singleton** - для створення єдиного екземпляру класу Application
- **Factory Method** - для створення об'єктів, що відображають дані

## Структура проекту

### Класи та інтерфейси
- **Application** - клас, що реалізує шаблон Singleton та запускає обробку команд
- **Command** - базовий інтерфейс для всіх команд
- **ConsoleCommand** - інтерфейс для консольних команд
- **Menu** - клас, що реалізує макрокоманду (колекцію команд)
- **Main** - містить статичний метод main() для запуску програми

### Реалізації команд
- **ViewConsoleCommand** - команда для відображення даних
- **GenerateConsoleCommand** - команда для генерації нових даних
- **ChangeConsoleCommand** - команда для зміни параметрів
- **SaveConsoleCommand** - команда для збереження даних
- **RestoreConsoleCommand** - команда для відновлення даних

## Взаємодія з користувачем
Програма відображає меню з доступними командами:
...
# Шосте завдання: Паралельне виконання

## Опис проекту
Даний модуль реалізує шаблон Worker Thread для обробки колекцій даних у багатопотоковому режимі.

## Функціональність
- Обчислення максимального значення в колекції (MaxCommand)
- Обчислення середнього значення елементів колекції (AvgCommand)
- Пошук мінімального позитивного та максимального негативного значень (MinMaxCommand)
- Використання кількох потоків для одночасної обробки різних задач
- Реалізація черги команд для потоків (CommandQueue)

## Основні класи та інтерфейси
- `Queue` - інтерфейс для реалізації шаблону Worker Thread
- `CommandQueue` - реалізація черги команд для виконання в окремому потоці
- `MaxCommand` - команда пошуку максимального значення
- `AvgCommand` - команда обчислення середнього значення
- `MinMaxCommand` - команда пошуку мінімального позитивного та максимального негативного значень
- `ExecuteConsoleCommand` - консольна команда для запуску всіх потоків обробки
- `Main` - головний клас додатку

## Шаблони проектування
- Worker Thread - для виконання задач у окремих потоках
- Command - для інкапсуляції запитів як об'єктів
- Factory Method - для створення об'єктів без прив'язки до конкретних класів
